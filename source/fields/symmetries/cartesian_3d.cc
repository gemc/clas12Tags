			...
			for (int i3 = 0; i3 < np_3; i3++) {
				...
				if (items_read == 6) {
					...
					if (verbosity > 4 && verbosity != 99) {
						cout << "  Loading Map: coordinates (" << d1 << ", " << d2 << ", " << d3 << ")   values: ("
							 << b1 / tesla << ", " << b2 / tesla << ", " << b3 / tesla << ") tesla";
					...
					if (feof(fp)) {
						...
						cout << "  !! Error:  reading field map, not enough values in line " << i1 << " " << i2 << " "
							 << i3 << endl;
					}
				}
			}
...
void gMappedField::GetFieldValue_cartesian3d(const double x[3], double *Bfield, int FIRST_ONLY) const {
	...
	if (symmetry == "cartesian_3D") {
		...
		if (XX < 0 || YY < 0) {
			cout << "xx " << xx << " yy " << yy << " zz " << zz << " XX " << XX << " YY " << YY << " ZZ " << ZZ << endl;
			return;
		}
	}
