...
MPrimaryGeneratorAction::MPrimaryGeneratorAction(goptions *opts) {
	...
	if (NP > 0) {
		...
		cout << endl << hd_msg << " Luminosity Particle Vertex: (" << L_vx / cm << ", " << L_vy / cm << ", "
			 << L_vz / cm << ") cm";
	...
	if (NP2 > 0) {
		...
		cout << endl << hd_msg << " Luminosity Particle Vertex: (" << L2_vx / cm << ", " << L2_vy / cm << ", "
			 << L2_vz / cm << ") cm";
...
void MPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent) {
	...
	if (input_gen == "gemc_internal") {
		if (cosmics == "no") { // internal, no cosmic
			...
			double Theta = acos(G4UniformRand() * (cos(theta / rad - dtheta / rad) - cos(theta / rad + dtheta / rad)) +
								cos(theta / rad + dtheta / rad)) / rad;
			if (primaryFlat) {
			...
			if (gemcOpt->optMap["ALIGN_ZAXIS"].args == "no") {
				beam_dir = G4ThreeVector(cos(Phi / rad) * sin(Theta / rad), sin(Phi / rad) * sin(Theta / rad),
										 cos(Theta / rad));
			} else if (gemcOpt->optMap["ALIGN_ZAXIS"].args == "beamp") {
				beam_dir = G4ThreeVector(cos(phi / rad) * sin(theta / rad), sin(phi / rad) * sin(theta / rad),
										 cos(theta / rad));
				const G4ThreeVector beam_axis(cos(phi / rad) * sin(theta / rad), sin(phi / rad) * sin(theta / rad),
											  cos(theta / rad));
				...
				beam_dir = G4ThreeVector(cos(cphi / rad) * sin(ctheta / rad), sin(cphi / rad) * sin(ctheta / rad),
										 cos(ctheta / rad));
			...
			if (GEN_VERBOSITY > 3) {
				cout << hd_msg << " Particle id=" << Particle->GetParticleName()
					 << "  Vertex=" << beam_vrt / cm << "cm,  momentum=" << Mom / GeV << " GeV, Kinematic Energy="
					 << akine / GeV << " GeV, theta="
			...
			if (cosmicNeutrons) {
				if (cminp < 0.1 || cmaxp > 10000)
					cout << "WARNING !!!! COSMIC NEUTRONS E (MeV) is OUT OF THE VALID RANGE !!!" << endl;
			...
			if (GEN_VERBOSITY > 3) {
				cout << hd_msg << " Particle id=" << Particle->GetParticleName()
					 << "  Vertex=" << G4ThreeVector(pvx, pvy, pvz) / cm << "cm,  momentum=" << thisMom / GeV
					 << " GeV, theta="
		...
		if ((gformat == "LUND" || gformat == "lund") && !gif.eof()) {
			...
			for (int p = 0; p < nparticles; p++) {
				...
				bool isNan =
						isnan(p) && isnan(pindex) && isnan(type) && isnan(pdef) && isnan(px) && isnan(py) &&
						isnan(pz) && isnan(Vx) && isnan(Vy) && isnan(Vz);
			...
			for (int p = 0; p < NPART; p++) {
				if (stdhep_reader->daughter1(p) > 0) {
					...
					if (!Particle) {
						cout << hd_msg << " Particle id " << stdhep_reader->pid(p) << " not found in G4 table." << endl
							 << endl;
					...
					// 4-momenta
					G4ThreeVector pmom(stdhep_reader->Px(p) * GeV, stdhep_reader->Py(p) * GeV,
									   stdhep_reader->Pz(p) * GeV);
					double Mom = pmom.mag();
					...
					G4ThreeVector beam_dir(cos(Phi / rad) * sin(Theta / rad), sin(Phi / rad) * sin(Theta / rad),
										   cos(Theta / rad));
					...
					particleGun->GeneratePrimaryVertex(anEvent);
					if (GEN_VERBOSITY > 3)
						cout << hd_msg << " Particle Number:  " << p << ", id=" << stdhep_reader->pid(p) << "("
		...
		for (int p = 0; p < nparticles; p++) {
			...
			if (pindex == p + 1) {
				...
				particleGun->SetParticleMomentumDirection(
						G4ThreeVector(cos(Phi / rad) * sin(Theta / rad), sin(Phi / rad) * sin(Theta / rad),
									  cos(Theta / rad)));
				...
				particleGun->GeneratePrimaryVertex(anEvent);
				if (GEN_VERBOSITY > 3)
					cout << hd_msg << " Merged Particle Number:  " << p + 1 << ", id=" << pdef << " ("
						 << Particle->GetParticleName() << ")"
						 << "  Vertex=" << beam_vrt / cm << "cm,  momentum=" << pmom / GeV << " GeV" << endl;
			} else if (pindex != p + 1 || pindex == 0) {
				if (GEN_VERBOSITY > 3) {
					cerr << hd_msg << " ERROR: file particle index " << pindex << " does not match read particle index "
						 << p + 1 << endl;
				}
				exit(1);
			}
	...
	if (NP > 0) {
		...
		for (int b = 0; b < NBUNCHES; b++) {
			...
			if (L_dmom > 0) {
				...
				L_Theta = acos(G4UniformRand() *
							   (cos(L_theta / rad - L_dtheta / rad) - cos(L_theta / rad + L_dtheta / rad)) +
			...
			particleGun->SetParticleMomentumDirection(
					G4ThreeVector(cos(L_Phi / rad) * sin(L_Theta / rad), sin(L_Phi / rad) * sin(L_Theta / rad),
								  cos(L_Theta / rad)));
	...
	if (NP2 > 0) {
		...
		for (int b = 0; b < NBUNCHES2; b++) {
			...
			if (L2_dmom > 0) {
				...
				L2_Theta = acos(G4UniformRand() *
								(cos(L2_theta / rad - L2_dtheta / rad) -
								 cos(L2_theta / rad + L2_dtheta / rad)) +
			...
			particleGun->SetParticleMomentumDirection(
					G4ThreeVector(cos(L2_Phi / rad) * sin(L2_Theta / rad),
								  sin(L2_Phi / rad) * sin(L2_Theta / rad),
								  cos(L2_Theta / rad)));
...
void MPrimaryGeneratorAction::setBeam() {
	...
	if (input_gen == "gemc_internal") {
		...
		if (cosmics == "no") {
			...
			if (!Particle) {
				...
				if (pname == "show_all") {
					...
					cout << hd_msg << " Particle " << pname << " not found in G4 table. Exiting" << endl
						 << endl;
				}
				exit(1);
			}
		...
		cout << hd_msg << "LUND: Opening  " << gformat << " file: " << trimSpacesFromString(gfilename).c_str()
			 << endl;
		gif.open(trimSpacesFromString(gfilename).c_str());
		if (!gif) {
			cerr << hd_msg << " Can't open LUND input file " << trimSpacesFromString(gfilename).c_str()
				 << ". Exiting. " << endl;
			exit(201);
		}
		...
		gformat.assign(input_gen, 0, input_gen.find(","));
		gfilename.assign(input_gen, input_gen.find(",") + 1, input_gen.size());
		cout << hd_msg << "BEAGLE: Opening  " << gformat << " file: " << trimSpacesFromString(gfilename).c_str()
			 << endl;
		gif.open(trimSpacesFromString(gfilename).c_str());
		if (!gif) {
			cerr << hd_msg << " Can't open BEAGLE input file " << trimSpacesFromString(gfilename).c_str()
				 << ". Exiting. " << endl;
			exit(202);
		}
		beagleHeader = 0;
		...
		gformat.assign(input_gen, 0, input_gen.find(","));
		gfilename.assign(input_gen, input_gen.find(",") + 1, input_gen.size());
		cout << hd_msg << "StdHEP: Opening  " << gformat << " file: " << trimSpacesFromString(gfilename).c_str()
			 << endl;
		stdhep_reader = new lStdHep(trimSpacesFromString(gfilename).c_str());

		if (!stdhep_reader) {
			cerr << hd_msg << " Can't open input file " << trimSpacesFromString(gfilename).c_str()
				 << ". Exiting. "
				 << endl;
			exit(203);
		}
	...
	if (background_gen != "no") {
		...
		if (!bgif.is_open()) {
			...
			if (!bgif) {
				cerr << hd_msg << " Can't open background input file >"
					 << trimSpacesFromString(background_gen).c_str() << "<. Exiting. " << endl;
				exit(204);
			}
		}
	}
...
void
MPrimaryGeneratorAction::setParticleFromPars(int p, int pindex, int type, int pdef, double px, double py,
											 double pz,
...
void MPrimaryGeneratorAction::setParticleFromParsPropagateTime(int p, vector <userInforForParticle> Userinfo,
															   G4Event *anEvent, int A, int Z) {
	...
	if (type == 1 && pindex == p + 1) {
		...
		if (GEN_VERBOSITY > 3) {
			cout << hd_msg << " Particle Number:  " << p + 1 << ", id=" << pdef << " ("
				 << Particle->GetParticleName() << ")"
				 << "  Vertex=" << beam_vrt / cm << "cm,  momentum=" << pmom / GeV << " GeV" << endl;
		}
		...
		if (eventIndex > ntoskip) {
			...
			if (parentindex[0] != 0 && Userinfo[parentindex[0] - 1].infos[2] != 1) {
				...
				double vertex_diff = sqrt(
						pow(Vx - Vx_parent, 2) + pow(Vy - Vy_parent, 2) + pow(Vz - Vz_parent, 2));
				//set parent momentum
