...
map<string, G4Material *> buildMaterials(map <string, materialFactory> materialFactoryMap, goptions go, runConditions rc) {
    ...
    // adding SQLITE
    materials *sqliteFactory = getMaterialFactory(&materialFactoryMap, "SQLITE");
    map < string, G4Material * > sqliteMats = sqliteFactory->initMaterials(rc, go);
    for (map<string, G4Material *>::iterator it = sqliteMats.begin(); it != sqliteMats.end(); it++) {
        mats[it->first] = it->second;
    }
