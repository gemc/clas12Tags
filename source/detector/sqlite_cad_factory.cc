...
map <string, detector> sqlitecad_det_factory::loadDetectors() {
	...
	for (map<string, detectorCondition>::iterator it = RC.detectorConditionsMap.begin();
		 it != RC.detectorConditionsMap.end(); it++) {
		...
		if (run_number == -1) {
			cout << hd_msg << " !!! Detector \"" << dname
				 << "\" not found in the SQLITE CAD database for requested run number <" << run << ">. Exiting."
				 << endl;
			exit(1);
		}
		...
		if (verbosity) {
			cout << hd_msg << " Importing SQLITE CAD Detector: " << dname << " with <" << factoryType
				 << "> factory, variation \"" << variation << "\", run number requested: " << run
				 << ",  sqlite run number used: " << run_number << endl;
		}
		...
		if (!q.exec(dbexecute.c_str())) {
			cout << hd_msg << " !!! Failed to execute CAD SQLITE query " << dbexecute
		...
		if (q.size() == 0 && verbosity) {
			cout << "  ** WARNING: detector \"" << dname << "\" not found with variation \"" << variation
		...
		while (q.next()) {
			...
			// Extract the last subdirectory
			// This first try is for the local cad dir.
			// Need to be careful here because usually the local cad dir is 'cad'
			size_t pos = cad_subdir.find_last_of('/');
