	...
	if (VERB > 4) {
		if (backgroundHits != nullptr) {
			for (auto sDet : SeDe_Map) {
				map<int, vector<BackgroundHit*>>* backgroundHitsEventMap = backgroundHits->getBackgroundForSystem(
					sDet.first);
				if (backgroundHitsEventMap != nullptr) {
					for (auto bgHits : (*backgroundHitsEventMap)) {
						if (bgHits.first == evtN) {
							cout << " >>> Background hits for detector " << sDet.first << ", event number: " << bgHits.
								first << endl;
	...
	if (RFSETUP != "no" && fastMCMode == 0) {
		...
		if (rfstartoption.size() == 4) {
			...
			if (rfstartoption[0] == "eventVertex") {
				G4ThreeVector referenceRFPosition(get_number(rfstartoption[1]), get_number(rfstartoption[2]),
												  get_number(rfstartoption[3]));
				G4ThreeVector firstParticleVertex = evt->GetPrimaryVertex(0)->GetPosition();
				additionalTime                    = (firstParticleVertex - referenceRFPosition).mag() / mm / 299.792;
			...
			if (WRITE_INTDGT.find(hitType) == string::npos && (fastMCMode == 0 || fastMCMode > 9)) {
				...
				for (int h = 0; h < nhits; h++) {
					...
					if (hitProcessRoutine->writeHit) {
						...
						if (VERB > 4) {
							cout << " Event Action: hit " << h + 1 << " was rejected in " << hitType <<
					...
					if (VERB > 6 || vname.find(catch_v) != string::npos) {
						cout << hd_msg << " Hit " << h + 1 << " --  total number of steps this hit: " << aHit->GetPos().
							size() << endl;
						cout << aHit->GetId();
						double Etot = 0;
	...
	*lundOutput << (int)bgMap.size() << "\t" << evtN << "\t 0 0 0 0 0 0 0 0 " << endl;
	...
	for (map<int, BGParts>::iterator it = bgMap.begin(); it != bgMap.end(); it++)
		*lundOutput << i++ << "\t0\t1\t" << it->second.pid << "\t0\t" << it->first << "\t"
			<< it->second.p.x() / GeV << "\t" << it->second.p.y() / GeV << "\t" << it->second.p.z() / GeV << "\t" << it
			->second.time << "\t0\t"
...
void MEventAction::setup_clas12_RF(int runno) {
	if (last_runno != runno) {
		...
		snprintf(database, sizeof(database), "%s:%d:%s%s", "/calibration/eb/rf/config", runno, digiVariation.c_str(),
				 timestamp.c_str());
		...
		snprintf(database, sizeof(database), "%s:%d:%s%s", "/calibration/eb/rf/offset", runno, digiVariation.c_str(),
				 timestamp.c_str());
		cout << " Connecting to " << database << endl;
		data.clear();
		calib->GetCalib(data, database);
