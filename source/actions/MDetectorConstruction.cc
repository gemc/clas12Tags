    ...
    for (map<string, detector>::iterator it = hallMap->begin(); it != hallMap->end(); it++) {
        ...
        if (mirrorString != "no") {
            ...
            if (itr != mirs->end()) {
                ...
                if (surfaceFinish == "polishedlumirrorglue")
                    mirrorSurfaces.back()->SetFinish(polishedlumirrorglue);  // mechanically polished surface, with lumirror & meltmount
        ...
        for (map<string, sensitiveDetector *>::iterator itr = SeDe_Map.begin(); itr != SeDe_Map.end(); itr++) {
            ...
            if (regionDet.system == itr->second->SDID.system) {
                ...
                if (itrr == SeRe_Map.end()) {
                    ...
                    if (VERB > 3) {
                        cout << "  Region " << regionName << " activated for volume " << regionDet.name << " with range: " << itr->second->SDID.prodThreshold
                             << endl;
                    }
        ...
        while (relatives.size() > 0) {
            ...
            if (kid.scanned == 0 && mom.scanned == 1) {
                ...
                if (kid.type.find("CopyOf") == 0) {
                    ...
                    if (dorig.scanned == 0) {
                        if (dorig.factory == "TEXT" || dorig.factory == "SQLITE") {
                ...
                if (mom.factory == "TEXT" || mom.factory == "SQLITE") {
