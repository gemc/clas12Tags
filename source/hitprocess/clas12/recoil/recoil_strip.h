...
public:
	...
	inline double get_strip_width( [[ maybe_unused ]] int strip) {
		double width;
		width = strip_width[0];
...
public:
	...
	vector<recoil_strip_found> FindStrip(G4ThreeVector xyz, double Edep, recoilConstants recoilc, double time);
	// Strip Finding Routine
	void   Carac_strip(int strip, recoilConstants recoilc); //length of the strip
	double Weight_td(int strip, double x, double y, double z, recoilConstants recoilc);
	//Compute the fraction of Nel falling onto the strip, depending on x,y in the FMT coordinate system
