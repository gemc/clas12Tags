...
static ftofConstants initializeFTOFConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false) {
	...
	snprintf(ftc.database, sizeof(ftc.database), "/calibration/ftof/time_walk_exp:%d:%s%s", ftc.runNo,
	...
	snprintf(ftc.database, sizeof(ftc.database), "/calibration/ftof/time_walk_pos:%d:%s%s", ftc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear();
	calib->GetCalib(data, ftc.database);
	for (unsigned row = 0; row < data.size(); row++) {
	...
	snprintf(ftc.database, sizeof(ftc.database), "/calibration/ftof/time_jitter:%d:%s%s", ftc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
...
map<string, double> ftof_HitProcess::integrateDgt(MHit* aHit, int hitn) {
	...
	if(ftc.jitter_cycles != 0) {
		tdc_jitter = ftc.jitter_period * ((0 + ftc.jitter_phase) % ftc.jitter_cycles);
	}
	double time_in_ns = 0;
	...
	if (ene > 0) {
		...
		time_in_ns  = G4RandGauss::shoot(tU + timeWalk + timeWalkE - timeWalkP,
										 sqrt(2) * ftc.tres[sector - 1][panel - 1][paddle - 1]);
