...
static htccConstants initializeHTCCConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false)
{
	...
	cout<<"HTCC:Getting gain"<<endl;
	snprintf(htccc.database, sizeof(htccc.database), "/calibration/htcc/gain:%d:%s%s", htccc.runNo, digiVariation.c_str(), timestamp.c_str());
	data.clear() ; calib->GetCalib(data,htccc.database);
	for(unsigned row = 0; row < data.size(); row++) {
		isec   = data[row][0]; ilay   = data[row][1];
		htccc.gain[isec-1][ilay-1].push_back(data[row][3]);
	}
	...
	snprintf(htccc.database, sizeof(htccc.database), "/calibration/htcc/ring_time:%d:%s%s", htccc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear();
	calib->GetCalib(data, htccc.database);
	for(unsigned row = 0; row < data.size(); row++) {
