...
static cndConstants initializeCNDConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false)
{
	...
	snprintf(cndc.database, sizeof(cndc.database), "/calibration/cnd/time_jitter:%d:%s%s", cndc.runNo,
...
map<string, double> cnd_HitProcess :: integrateDgt(MHit* aHit, int hitn)
{
	...
	double time_in_ns = 0;
	double tdc_jitter = 0;
	if(cndc.jitter_cycles != 0) {
		tdc_jitter = cndc.jitter_period * ((0 + cndc.jitter_phase) % cndc.jitter_cycles);
	}
