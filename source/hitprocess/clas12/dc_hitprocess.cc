...
static dcConstants initializeDCConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false)
{
	...
	snprintf(dcc.database, sizeof(dcc.database),  "/calibration/dc/v2/ref_pressure:%d:%s%s", dcc.runNo,
	...
	snprintf(dcc.database, sizeof(dcc.database),  "/calibration/dc/v2/t2d_pressure:%d:%s%s", dcc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear();
	calib->GetCalib(data, dcc.database);
	...
	snprintf(dcc.database, sizeof(dcc.database),  "/calibration/dc/v2/t0:%d:%s%s", dcc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear();
	calib->GetCalib(data,  dcc.database);
	...
	snprintf(dcc.database, sizeof(dcc.database),  "/calibration/dc/time_jitter:%d:%s%s", dcc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear();
	calib->GetCalib(data, dcc.database);
...
//	cout << nsteps << " " << (SECI+1) << "/" << (SLI+1) << "/" << (LAY+1) << "/" << nwire << " dim: " << ylength <<"/"<< xlength_low << "/" << xlength_high <<endl;
	...
	if(dcc.jitter_cycles != 0) {
		tdc_jitter = dcc.jitter_period * ((0 + dcc.jitter_phase) % dcc.jitter_cycles);
	}
...
//		  cout << " tprop: " << tprop << " ylength: " << ylength << " WIRE_Y: " << WIRE_Y << " WIRE_DX: " << WIRE_DX << "/" << xlength_low+(xlength_high-xlength_low)*(Lpos[s].y()+ylength)/2/ylength << " x: " << (Lpos[s].x()-WIRE_X) << " y: " << (Lpos[s].y()+ylength) << " Lposx: " << (Lpos[s].x()) << " Lposy: " << (Lpos[s].y()) << " Lposz: " << Lpos[s].z() << " phi: " << atan2(pos[s].y(),pos[s].x())*180/3.14156 << " dcc.NWIRES: " << dcc.NWIRES << endl;
	...
	unsmeared_time += calc_TimeBeta(doca/cm, beta_particle, SECI, SLI);
	...
	double smeared_time = unsmeared_time + dt_random + hit_signal_t + prop_t +
...
double dc_HitProcess :: calc_TimeBeta(double x, double beta, int sector, int superlayer)
{
	...
	return (0.5*pow(beta*beta*distbeta, 3)*x/(pow(beta*beta*distbeta, 3)+x*x*x))/v0;
}
