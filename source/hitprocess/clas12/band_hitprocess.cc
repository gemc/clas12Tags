...
static bandHitConstants initializeBANDHitConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", [[maybe_unused]] bool accountForHardwareStatus = false)
{
	...
	snprintf(bhc.database, sizeof(bhc.database), "/calibration/band/time_jitter:%d:%s%s", bhc.runNo,
...
map<string, double> band_HitProcess :: integrateDgt(MHit* aHit, int hitn)
{
	...
	double tdc_jitter = 0;
	if(bhc.jitter_cycles != 0) {
		tdc_jitter = bhc.jitter_period * ((0 + bhc.jitter_phase) % bhc.jitter_cycles);
	}
...
map< int, vector <double> > band_HitProcess :: chargeTime([[maybe_unused]] MHit* aHit, int hitn)
{
