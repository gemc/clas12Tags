...
static ctofConstants initializeCTOFConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false) {
	...
	if(accountForHardwareStatus) {
		snprintf(ctc.database, sizeof(ctc.database),  "/calibration/ctof/status:%d:%s%s", ctc.runNo,
	...
	snprintf(ctc.database, sizeof(ctc.database), "/calibration/ctof/time_jitter:%d:%s%s", ctc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
...
map<string, double> ctof_HitProcess::integrateDgt(MHit* aHit, int hitn)
{
	...
	if (ene > 0) {
		...
		// tdcu = tU / tdcconv;
		double tdc_jitter = 0;
		if(ctc.jitter_cycles != 0) {
			tdc_jitter = ctc.jitter_period * ((0 + ctc.jitter_phase) % ctc.jitter_cycles);
		}
