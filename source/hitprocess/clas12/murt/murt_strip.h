...
public:
	...
	vector<double> FindStrip( int layer, int sector, G4ThreeVector lxyz, double Edep, murtConstants murtc);   // Strip Finding Routine
	
	double getSigma( int layer, double x, double y, murtConstants murtc);     // sigma for C-detector
	int getClosestStrip( int layer, int sector, double angle, double z,murtConstants murtc);
	int getStripGroup(int layer, int strip, murtConstants murtc);
	double GetStripInfo(int layer, int sector, int strip, murtConstants murtc); 				   // the z position of a given C strip. Not used?
	int isInSector(int layer, double angle, murtConstants murtc);
	double Weight_td(int layer, int strip, double angle, double z, murtConstants murtc); //Compute the likelihood to get an electron
