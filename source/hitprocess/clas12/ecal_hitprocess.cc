...
static ecConstants initializeECConstants(int runno, string digiVariation = "default", string digiSnapshotTime = "no", bool accountForHardwareStatus = false)
{
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/atten:%d:%s%s", ecc.runNo,
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/ftime:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/dtime:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/fadc_global_offset:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data, ecc.database);
	ecc.fadc_global_offset = data[0][3];

	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/global_time_walk:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data, ecc.database);
	for(unsigned row = 0; row < data.size(); row++)
	{
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/time_jitter:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear();
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/fadc_offset:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data, ecc.database);
	for(unsigned row = 0; row < data.size(); row++)
	{
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/tmf_offset:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data, ecc.database);
	for(unsigned row = 0; row < data.size(); row++)
	{
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/fveff:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/dveff:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/deff:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/fthr:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/ftres:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
	...
	snprintf(ecc.database, sizeof(ecc.database), "/calibration/ec/dtres:%d:%s%s", ecc.runNo,
			 digiVariation.c_str(), timestamp.c_str());
	data.clear(); calib->GetCalib(data,ecc.database);
...
map<string, double> ecal_HitProcess :: integrateDgt(MHit* aHit, int hitn)
{
	...
	if(ecc.jitter_cycles != 0) {
		tdc_jitter = ecc.jitter_period * ((0 + ecc.jitter_phase) % ecc.jitter_cycles);
	}
		...
		if (EC_npe>0) {
			...
			if (EC_GeV>0) {
				...
				double ftim = (fa4==0||fa6==0) ? 0 :
							  fa0 + fa2 + exp(-(radc-fa3)/fa4) + 1 - exp((radc-fa5)/fa6);
				double dtim = (da4==0||da6==0||da7==0) ? 0 :
							  da0 + gtw/radc + da2 + exp(-(radc-da3)/da4) + 1 -
							  exp(-(da5-radc)/da6) - exp(-(radc-da3*0.95)/da7)*pow(radc, da8);
	...
	if (ecc.outputRAW==0 && def0>0 && dtime_in_ns > 0 &&
		G4UniformRand() > 1/pow(1+exp(-def0*(ADC/10-def1)),def2)) {
		dtime_in_ns = 0; // DSC/TDC threshold
	}
