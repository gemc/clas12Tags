...
our $PATH_URWT_MAT    = "/test/urwt/urwt_material";
our $PATH_URWT_GLOBAL = "/test/urwt/urwt_global";
...
# Filled from CCDB by init_urwt_from_ccdb()
...
# Da chiamare da urwt.pl PRIMA di make_urwt()
sub init_urwt_from_ccdb {
    ...
    %global         = parse_urwt_global($rows_global);

    $NREGIONS = int($global{NLayers} / 2.0 + 0.5);
    $NSECTORS = $global{Nsectors};

    # Debug opzionale
    print "=== urwt_global (variation=$VARIATION) ===\n";
...
sub make_urwt {
    for (my $R = 1; $R <= $NREGIONS; $R++) {
        for (my $S = 1; $S <= $NSECTORS; $S++) {
            ...
            make_urwt_structure($R, $S);
        }
    }
}

sub make_urwt_structure {
    ...
    for my $layer (sort { $a <=> $b } keys %layer_material) {
        for my $component (sort { $a <=> $b } keys %{ $layer_material{$layer} }) {
            ...
            my %detector = main::init_det();
            my $vname = "rg_urwt_${iRegion}_s${iSector}_l${ilayer}_matC${component}";
            ...
            if ($sens eq '1') {
                $detector{"sensitivity"} = "urwt";
                $detector{"hit_type"}    = "urwt";
...
sub parse_urwt_global {
    my ($rows) = @_;
    die "urwt_global is expected to have at least one row\n"
      unless @$rows;
