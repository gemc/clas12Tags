...
// Why run #19?
//ConstantProvider cp = GeometryFactory.getConstants(DetectorType.MUCAL, 19, "default");

// ConstantProvider cp = GeometryFactory.getConstants(DetectorType.MUCAL, runNumber, variation);

MUCALGeant4Factory factory = new MUCALGeant4Factory(null);

def outFile = new File("mucal__volumes_"+variation+".txt");
outFile.newWriter().withWriter { w ->
	w<<factory;
}

def axisName = ["x", "y", "z"];
def parFile = new File("mucal__parameters_"+variation+".txt");
...
for(panels in factory.getMother().getChildren()) {
  if(panels.getName().contains("s1")) {
	writer<<sprintf("%s | %d | %s | %s | %s | %s | %s | %s | %s\n",
		"mucal.panel"+panels.getName().split("_")[1].substring(1)+".ncounters",	//ncounters
