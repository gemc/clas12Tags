...
class GeoArgParse {
    ...
    static int getNumberOfRegion(String[] args) {
        for (int ii=0; ii<args.length-1; ii++) {
            if (args[ii].equals("--number_of_Region")) return Integer.parseInt(args[ii+1]);
        }
        throw new RuntimeException("GeoArgParse:  '--number_of_Region number_of_Region' is required.");
    }
