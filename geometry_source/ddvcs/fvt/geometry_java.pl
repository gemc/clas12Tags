...
sub make_region_proto
{
    ...
    for(my $s=1; $s<=$nSectors; $s++)
    {
        ...
        my $vname = "region_uRwell_$R"."_s$ss"."_$config";
        ...
        make_chamber($R,$ss,$nChambers,$isProto,$config);
...
sub make_region
{
	...
	for(my $ss=1; $ss<=$nSectors; $ss++)
	{
		...
		my %detector = init_det();
		my $vname = "region_uRwell_$R"."_s$s"."$config";
		...
		$detector{"visible"}     = 0;
		print_det(\%main::configuration, \%detector);
        make_chamber($R,$s,$nChambers,$isProto, $var);
...
sub make_chamber
{
    ...
    for(my $c=1; $c<=$nChambers; $c++){
        my %detector = init_det();
        my $vname = "rg$iRegion"."_s$isector"."_c$c"."$config";
        ...
        make_layers($iRegion, $isector, $c, "window",$window[$ii], $isProto, $var);
        }
        
        # Cathode
        for(my $ii=0; $ii<=$#cathode; $ii++) {
        make_layers($iRegion, $isector, $c, "cathode",$cathode[$ii], $isProto,  $var);
        }
        
        #muRwell
        for(my $ii=0; $ii<=$#muRwell; $ii++) {
        make_layers($iRegion, $isector, $c, "muRwell",$muRwell[$ii], $isProto, $var);
        }
        
        #@capa_sharing_layer1
        for(my $ii=0; $ii<=$#capa_sharing_layer1; $ii++) {
        make_layers($iRegion, $isector, $c, "capa_sharing_layer1",$capa_sharing_layer1[$ii], $isProto, $var);
        }
        
        #@capa_sharing_layer2
        for(my $ii=0; $ii<=$#capa_sharing_layer2; $ii++) {
        make_layers($iRegion, $isector, $c, "capa_sharing_layer2",$capa_sharing_layer2[$ii], $isProto, $var);
        }
        
        #@readout1
        for(my $ii=0; $ii<=$#readout1; $ii++) {
        make_layers($iRegion, $isector, $c, "readout1",$readout1[$ii], $isProto, $var);
        }

        #@readout2
        for(my $ii=0; $ii<=$#readout2; $ii++) {
        make_layers($iRegion, $isector, $c, "readout2",$readout2[$ii], $isProto, $var);
        }

        #@readout3
        for(my $ii=0; $ii<=$#readout3; $ii++) {
        make_layers($iRegion, $isector, $c, "readout3",$readout3[$ii], $isProto, $var);
        }
        
        #@support_skin1
        for(my $ii=0; $ii<=$#support_skin1; $ii++) {
        make_layers($iRegion, $isector, $c, "support_skin1",$support_skin1[$ii], $isProto, $var);
        }

        #@support_skin2
        for(my $ii=0; $ii<=$#support_skin2; $ii++) {
        make_layers($iRegion, $isector, $c, "support_skin2",$support_skin2[$ii], $isProto, $var);
        }
        
        #@support_honeycomb
        for(my $ii=0; $ii<=$#support_honeycomb; $ii++) {
        make_layers($iRegion, $isector, $c, "support_honeycomb",$support_honeycomb[$ii], $isProto, $var);
        }
...
sub make_layers{
    ...
    my $vname = "rg$iRegion"."_s$isector"."_c$nChamber"."_$layer"."_$material"."$config";
...
sub makeURWELL_PROTO
{
    ...
    make_region_proto($NRegion,$config);
