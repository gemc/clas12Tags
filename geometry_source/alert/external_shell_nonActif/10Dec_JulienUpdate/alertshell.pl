...
sub make_ATOFfixBrass
{
	...
	$detector{"dimensions"}  = "$rmin*mm $rmax*mm $z_half_atof*mm $phistart*deg $pspan*deg";
	#$detector{"material"}    = $mate;
	$detector{"material"}    = "G4_Cu"; # adapt to brass = laiton if needed, rho = 8.730 g/cm3 instead of 8.96 g/cm3
	$detector{"style"}       = 1;
	$detector{"identifiers"} = $id_string;
	print_det(\%configuration, \%detector);
...
# there are 5 AL ribs and 10 DELMAT EPOXY 68660 ribs
sub make_ALribs
{
        my $rmin = 149.032;
	my $rmax  = 150.000;
...
sub make_EPOXYribs
{
	...
	$detector{"name"} = "EPOXYribs";
	#$detector{"mother"}      = "mother_shell";
	$detector{"mother"}      = "ahdc_mother";
	$detector{"description"} = "ATOF EPOXY ribs";
