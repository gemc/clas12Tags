...
This is the GEMC2 (clas12Tags) variant. Digitization, output, and field categories are DISCOVERED,
not hard-coded: every ``<det>_HitProcess::integrateDgt`` / ``::multiDgt``, ``<fmt>_output::writeEvent``,
and ``<...>Field::GetFieldValue`` present in the profile is reported as its own row, so each detector's
hit process appears automatically (ftof, dc, ecal, ltcc, ...).

Categories use inclusive cost and can overlap or nest (field evaluation is part of track swimming),
so the percentages are not meant to add up to 100%.
"""
...
FIXED_CATEGORIES = [
    ...
    ("Hit collection (sensitive detector)", re.compile(r"\bsensitiveDetector::ProcessHits\b")),
]

# Discovered category families: (family label, class-capturing regex, method label for display).
DISCOVERED_FAMILIES = [
    ("Digitization", re.compile(r"\b([A-Za-z_]\w*)::(?:integrateDgt|multiDgt)\b"), "integrateDgt/multiDgt"),
    ("Output writing", re.compile(r"\b([A-Za-z_]\w*_output)::writeEvent\b"), "writeEvent"),
    ("Field evaluation", re.compile(r"\b([A-Za-z_]\w*[Ff]ield\w*)::GetFieldValue\b"), "GetFieldValue"),
]
...
def location_to_func(location: str) -> str:
    """Reduce a callgrind ``file:function (Nx) [object]`` location to a readable routine name.

    Trailing ``[object]`` and the call-count ``(12,345x)`` suffix are removed, and an unresolved bare
    address (a symbol callgrind could not name) is labelled with its object instead of a bare hex.
    """
    obj = None
    previous_value = None
    while previous_value != location:
        previous_value = location
        match = re.search(r"\s*\[([^\]]*)\]\s*$", location)
        if match:
            obj = obj or match.group(1)
            location = location[: match.start()]
        location = re.sub(r"\s*\(\s*[\d,]+x\s*\)\s*$", "", location).rstrip()
    ...
    for index, char in enumerate(location):
        ...
        if previous != ":" and following != ":":
            func = location[index + 1:]
            break
    func = func.strip()

    if re.fullmatch(r"0x[0-9a-fA-F]+", func):
        func = f"{func} in {obj.rsplit('/', 1)[-1]}" if obj else f"{func} (unresolved)"
    return func


def is_named_routine(func: str) -> bool:
    """Drop callgrind_annotate summary artifacts (e.g. 'events annotated') that are not routines."""
    if "::" in func or "(" in func or "0x" in func:
        return True
    return " " not in func.strip()
...
def collect_rows(rows: list[tuple[str, dict[str, int]]]) -> list[tuple[str, str, int]]:
    ...
    for family, pattern, method in DISCOVERED_FAMILIES:
...
def render(title: str, total: dict[str, int], rows: list[tuple[str, dict[str, int]]]) -> str:
    ...
    lines.append("|----------|-----------------|---------------:|---------:|")
    for label, symbol, value in collect_rows(rows):
...
def top_routines(rows: list[tuple[str, dict[str, int]]], total_cest: int, count: int = 10) -> str:
    ...
    ranked = sorted(
        ((func, cest(counts)) for func, counts in rows if is_named_routine(func)),
