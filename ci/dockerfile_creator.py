...
def additional_software(image: str) -> str:
	...
	if image == "almalinux":
		return (
			...
			" && JAVAC21_BIN=$(rpm -ql java-21-openjdk-devel | grep -E '/bin/javac$' | head -n 1) \\\n"
			...
			"    fi \\\n"
			" && if alternatives --display javac >/dev/null 2>&1; then \\\n"
			"      alternatives --set javac \"${JAVAC21_BIN}\"; \\\n"
			"    else \\\n"
			"      alternatives --install /usr/bin/javac javac \"${JAVAC21_BIN}\" 21000; \\\n"
			"    fi \\\n"
			" && dnf -y clean all \\\n"
			" && rm -rf /var/cache/dnf\n"
		)
