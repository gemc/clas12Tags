...
def coatjava_deps(image: str) -> str:
	...
	post_java_fix = ""
	if image in ("fedora", "almalinux"):
		# maven may pull java-17; force java back to 21 as the system default
		post_java_fix = (
			" && JAVA21_BIN=$(rpm -ql java-21-openjdk-headless 2>/dev/null | grep -E '/bin/java$' | head -n 1) \\\n"
			" && alternatives --set java \"${JAVA21_BIN}\" \\\n"
		)
	elif image in ("ubuntu", "debian"):
		# ensure java points to 21 if multiple JDKs exist
		post_java_fix = (
			" && update-alternatives --set java \"$(update-alternatives --list java | grep -E '/java-21|/jdk-21' | head -n 1)\" \\\n"
			" || true \\\n"
		)
